...
# ## phylogenetic ----
# b = 5
# sample2 <- iNEXT.link(data = network_list%>%lapply(function(x){round(x/10)}), diversity = 'PD', datatype = "abundance",
#                       nboot = b,col.tree = coltree, row.tree = rowtree)
#
#
# asy2 = Asy.link(data = network_list%>%lapply(function(x){round(x/10)}), diversity = 'PD', q = seq(0,2,0.25),nboot = b,
#                 col.tree = coltree, row.tree = rowtree)
# obs2 = Obs.link(data = network_list%>%lapply(function(x){round(x/10)}), diversity = 'PD', q = seq(0,2,0.25),nboot = b,
#                 col.tree = coltree, row.tree = rowtree)
# dissimilarity2 = iNEXTbeta.link(networks = network_list%>%lapply(function(x){round(x/10)}), level = seq(0.5,1,0.2),
#                                  datatype='abundance',
