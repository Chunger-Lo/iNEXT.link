...
`iNEXT.link` is a R package providing the rarefaction/extrapolation and asymptotic estimation to achieve fair comparison of network diversity among multiple assemblages (Hsieh and Chao, Systematic Biology, 2017). In this document, we provide a quick introduction demonstrating how to run `iNEXT.link`. The Detailed information about `iNEXT.link` functions is described in the `iNEXT.link` Manual, also available in [CRAN](https://cran.r-project.org/package=PhD). An online version of `iNEXT.link.Online` (<https://chao.shinyapps.io/iNEXT.link.Online/>) is developed for users without R background. The theoretical derivation of network diversity measurements was proposed in xxx. `iNEXT.link` is an extension for [`iNext`](https://cran.r-project.org/package=iNextPD), which computes diversity estimates and its `conf` confidence intervals for the two types of rarefaction and extrapolation (R/E) based on two different sampling data sets, individual-based abundance data and quadrat-based incidence data.

There are Four main functions in `iNEXT.link`, including `iNEXT_ND`, `AsyND`, `ObsND` and `estimateND`.

1. `iNEXT_ND`: provides a seamless rarefaction and extrapolation sampling curve for Chao et al.’s Network diversity. See Chao et al. (2010, 2015) and Hsieh and Chao (2017) for pertinent background and methods. It is the network extension of `iNEXT` function in `iNEXT` package.

2. `AsyND`: computes asymptotic network diversity estimates with respect to specified/default diversity order q to infer network diversity (ND) and phylogenetic network diversity (PND).

3. `ObsND`: computes empirical (i.e., observed) phylogenetic diversity (PD) and phylogenetic Hill numbers (meanPD, mean phylogenetic diversity) for specified/default order q and reference time; see Chao et al. (2010) for details of PD and meanPD

4. `estimateND`: computes Chao et al.’s (2010) phylogenetic diversity (PD, effective total branch lengths, for diversity order q = 0, 1 and 2) and mean phylogenetic diversity (meanPD, phylogenetic Hill numbers or the effective number of lineages, q = 0, 1 and 2) for given values of sample coverage. See Chao et al. (2010, 2015) and Hsieh and Chao (2017) for formulas and interpretations. It is the phylogenetic extension of `estimateD` in `iNEXT` package.
...
source("..//R//myfunc.R")
source("..//R//iNEXT_beta.R")
source("..//R//iNEXT3D_0316.R")
```

<br><font size="5"><b>DATA FORMAT/INFORMATION</b></font> 
-------------------
In iNEXT.link package, all four main functions share the same required format of `data`. 
Now, only abundance data is supported.

Individual‐based abundance data (`datatype="abundance"`): species-by-site matrix/data.frame of species abundances. The row (species) names of data must match the species names in the phylogenetic tree and thus cannot be missing. Users can use the following command to see an example data containing three.
...
```{r, eval=FALSE}
# AsyPND(data.abu, datatype = "abundance", tree, q = c(0,1,2), datatype = "abundance", nboot = 30)
```
...
<br><font size="5"><b>iNEXT.link VIA EXAMPLES: Network diversity Analysis</b></font>
-------------------

Here, we use an abundance demo data as an illustrating example.
...
### Main function: `iNEXT_PND()`: 

```{r,eval=T, message = F, warning=F}
sample1 <- iNEXT_ND(x = Norfolk, datatype = "abundance", nboot = 10)
```
...
```{r, eval = F}
kable(estimateND(dat, datatype="abundance", base="coverage",level=0.7, conf=0.95))
```
...
<br><font size="5"><b>EXAMPLES2: Abundance data with phylogenetic tree </b></font>
-------------------
...
```{r,echo = TRUE ,fig.width=7, fig.height=4, fig.align="center",warning=FALSE}
ggiNEXT_PND(sample2, type = 1,facet = "Order.q" )
```


```{r,echo = TRUE ,fig.width=7, fig.height=4, fig.align="center",warning=FALSE}
ggiNEXT_PND(sample2, type = 3, facet = "Assemblage")
```

<!-- ### 4. `estimateND()` -->

```{r, eval = F}
# kable(estimatePND(dat, datatype="abundance", base="coverage",level=0.7, conf=0.95))
```
...
<br><font size="5"><b>EXAMPLES3: Dissimilarity </b></font>
...
```{r,echo = TRUE ,fig.width=7, fig.height=4, fig.align="center",warning=FALSE}
ggiNEXT_beta(dissimilarity1, type = 'B')
ggiNEXT_beta(dissimilarity1, type = 'D')
```



```{r}
dissimilarity2 = iNEXT_beta_link(Norfolk, coverage_expected = seq(0.5,1,0.05), data_type='abundance',
                                 q = c(0, 1, 2),level='taxonomic', 
...
```{r,echo = TRUE ,fig.width=7, fig.height=4, fig.align="center",warning=FALSE}
ggiNEXT_beta(dissimilarity2, type = 'B')
ggiNEXT_beta(dissimilarity2, type = 'D')
```
