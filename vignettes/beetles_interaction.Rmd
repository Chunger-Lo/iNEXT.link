...
# bef = Sys.time()
# b = 50
# sample1 <- iNEXT.link(x = network_list, diversity = 'TD', datatype = "abundance", nboot = b)
...
size_view = re_size%>%
  filter(Order.q == 0)%>%
  filter(m < 10000)%>%
  ggplot(aes(x = m, y = qD, fill = Assemblage, color = Assemblage))+
  geom_point()+
  geom_line()
coverage_view = re_coverage%>%
  ...
  ggplot(aes(x = goalSC, y = qD, fill = Assemblage, color = Assemblage))+
  geom_point()+
  geom_line()
...
# dissimilarity_pool_focus = lapply(dissimilarity_pool[[1]], function(dis){
#   
#   dis%>%filter(Coverage_expected > 0.8)
# })
# ggiNEXT_beta.link(list(dissimilarity_pool_focus), type = 'B')
```
...
size_view = re_size%>%
  filter(Order.q == 0)%>%
  filter(m < 10000)%>%
  ggplot(aes(x = m, y = PD, fill = Region, color = Region))+
  geom_point()+
  geom_line()
